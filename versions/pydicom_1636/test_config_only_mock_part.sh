...
# NOTE: Using pytest<8.0 because pytest 8.x breaks setup()/teardown() methods
#       pytest 8.x requires setup_method()/teardown_method() instead
# NOTE: Uninstalling pytest-asyncio and pytest-httpx because they require pytest>=8.0
#       and conflict with pytest 7.x
SETUP_COMMAND="pip install --upgrade pip && pip uninstall -y pytest-asyncio pytest-httpx && pip install -e . && pip install 'pytest>=7.0,<8.0' pytest-timeout tqdm"
