#!/bin/bash
# Test configuration for ert_11206
...
TEST_COMMAND="uv run pytest  tests/ert/ui_tests/cli/test_cli.py::test_that_connection_errors_do_not_effect_final_result --durations=0 -vv"
...
SETUP_COMMAND="git init 2>/dev/null || true && export SETUPTOOLS_SCM_PRETEND_VERSION_FOR_ERT=11206.0.0 && uv sync --all-extras && git submodule update --init --recursive"
...
PROJECT_NAME="ert_11206"

# Export environment variable for setuptools-scm to avoid git requirement
export SETUPTOOLS_SCM_PRETEND_VERSION_FOR_ERT="11206.0.0"
